...
namespace symbol
{
	...
	****************************/
	...
	void SymbolTable<Entry>::insert(string lexeme, const Entry value)
	{
		...
		//check if already in first hash table in list aka scope. if yes, redefinition in same scope err

		if (localContains(lexeme)) {

			cout << "redefinition in same scope" << endl;
        }
		else {
			Iterator it = tables.begin();
			HashTable		&current = *it;
		...
		//insert into first hash table in list
	}
